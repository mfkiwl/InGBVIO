...
namespace ingvio
{
    ...
    void IngvioFilter::callbackGnssMeas(const gnss_comm::GnssMeasMsgConstPtr& gnss_meas_msg)
    {
        ...
        if (!_filter_params._enable_gnss || !_gnss_data->_isIono)
            {
                std::cout << "[GnssSync]: GNSS is not enabled or iono params are not ready." << std::endl;
                return;}
        ...
        //     double delta_time = _gnss_sync->getUnsyncTime(); 
        //     double gps_fix_time = gnss_comm::time2sec(valid_meas[0]->time) + delta_time;

        // // get lla
        //     Eigen::Vector3d llaspp = gnss_comm::ecef2geo(pos_ecef);
        //     sensor_msgs::NavSatFix lla_spp_msg;
        //     lla_spp_msg.header.stamp = ros::Time().fromSec(gps_fix_time);
        ...
        //     _gps_fix_meas->inputGPSFix(boost::make_shared<sensor_msgs::NavSatFix>(lla_spp_msg));
        //     std::cout << "[GnssSync]: SPP lla = " << std::setprecision(10) << llaspp.transpose() << std::endl;
