...
namespace feature_tracker
{
    ...
    void StereoTracker::rejectWithTwinRANSAC()
    {
        ...
        if (forw_left_pts.size() >= 8)
        {
            ...
            std::vector<uchar> status;
            cv::findFundamentalMat(un_cur_left_pts, un_forw_left_pts, cv::FM_RANSAC, param.F_threshold, 0.999, status);
        ...
        if (forw_right_pts.size() > 8)
        {
            ...
            std::vector<uchar> status;
            cv::findFundamentalMat(un_cur_right_pts, un_forw_right_pts, cv::FM_RANSAC, param.F_threshold, 0.999, status);
        ...
        if (origin_pts_num > 0 && param.print_track_info) //&& final_pts_num != origin_pts_num 
